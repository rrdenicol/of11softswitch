...
struct nicira_header {
    ...
    uint32_t subtype;           /* One of NXT_* below. */
};
...
struct nicira_stats_msg {
    ...
    uint16_t type;              /* OFPST_VENDOR. */
    uint16_t flags;             /* OFPSF_{REQ,REPLY}_*. */
    uint32_t vendor;            /* NX_VENDOR_ID. */
    uint32_t subtype;           /* One of NXST_* below. */
...
struct nx_flow_mod {
    ...
    uint16_t match_len;           /* Size of nx_match. */
...
struct nx_flow_stats_request {
    ...
    uint16_t out_port;        /* Require matching entries to include this
                                 ...
                                 indicates no restriction. */
    uint16_t match_len;       /* Length of nx_match. */
